...
.OUTPUTS
    Hashtable
#>
function Get-PVEHttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-PVEConfigurationHttpSigning
    $Digest = $null

    #get the body digest
    $bodyHash = Get-PVEStringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        if ($headerItem -eq $HEADER_REQUEST_TARGET) {
            ...
            $created = Get-PVEUnixTime -Date $dateTime -TotalTime TotalSeconds
            ...
            $expireEpocTime = Get-PVEUnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $signatureHashString = Get-PVEStringHash -String $headerValuesString -HashName $httpSigningConfiguration.HashAlgorithm

    #Gets the Key type to select the correct signing algorithm
    $KeyType = Get-PVEKeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-PVERSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
        ...
        $headerSignatureStr = Get-PVEECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
    #Deprecated
    <#$cryptographicScheme = Get-PVECryptographicScheme -SigningAlgorithm $httpSigningConfiguration.SigningAlgorithm `
...
function Get-PVERSASignature {
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "PVERSAEncryptionProvider.cs"
...
.OUTPUTS
    Base64String
#>
function Get-PVEECDSASignature {
    param(
...
function Get-PVECryptographicScheme {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SigningAlgorithm,
        [Parameter(Mandatory = $true)]
...
String
#>
function Get-PVEKeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
