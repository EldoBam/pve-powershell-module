...
function Initialize-PVEPOSTNodesQemuMovediskRB {
    ...
    Process {
        ...
        if (!$TargetDigest -and $TargetDigest.length -gt 40) {
		 ...
		 $DisplayNameMapping =@{
			"Storage"="storage"; "Disk"="disk"; "TargetDisk"="target-disk"; "Format"="format"; "TargetVmid"="target-vmid"; "Bwlimit"="bwlimit"; "Vmid"="vmid"; "Delete"="delete"; "Digest"="digest"; "TargetDigest"="target-digest"; "Node"="node"
        }
...
function ConvertFrom-PVEJsonToPOSTNodesQemuMovediskRB {
    ...
    Process {
        ...
        $AllProperties = ("storage", "disk", "target-disk", "format", "target-vmid", "bwlimit", "vmid", "delete", "digest", "target-digest", "node")
