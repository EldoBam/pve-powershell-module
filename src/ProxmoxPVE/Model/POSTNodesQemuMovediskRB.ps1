...
function Initialize-PVEPOSTNodesQemuMovediskRB {
    ...
    Process {
        ...
        if (!$TargetDigest -and $TargetDigest.length -gt 40) {
		 ...
		 $DisplayNameMapping =@{
			"Format"="format"; "TargetDisk"="target-disk"; "Disk"="disk"; "Digest"="digest"; "Bwlimit"="bwlimit"; "Vmid"="vmid"; "Node"="node"; "Storage"="storage"; "TargetVmid"="target-vmid"; "Delete"="delete"; "TargetDigest"="target-digest"
        }
...
function ConvertFrom-PVEJsonToPOSTNodesQemuMovediskRB {
    ...
    Process {
        ...
        $AllProperties = ("format", "target-disk", "disk", "digest", "bwlimit", "vmid", "node", "storage", "target-vmid", "delete", "target-digest")
