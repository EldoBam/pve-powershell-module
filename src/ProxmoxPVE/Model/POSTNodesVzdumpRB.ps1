...
function Initialize-PVEPOSTNodesVzdumpRB {
    ...
    Process {
        ...
        if ($Stdout -and $Stdout -lt 0) {
		 ...
		 $DisplayNameMapping =@{
			"Tmpdir"="tmpdir"; "Mode"="mode"; "Protected"="protected"; "NotificationMode"="notification-mode"; "Maxfiles"="maxfiles"; "All"="all"; "NotificationTarget"="notification-target"; "Mailto"="mailto"; "Ionice"="ionice"; "Stop"="stop"; "Zstd"="zstd"; "Mailnotification"="mailnotification"; "Performance"="performance"; "Node"="node"; "NotesTemplate"="notes-template"; "JobId"="job-id"; "Fleecing"="fleecing"; "Dumpdir"="dumpdir"; "ExcludePath"="exclude-path"; "Remove"="remove"; "Stdexcludes"="stdexcludes"; "Pool"="pool"; "Quiet"="quiet"; "Storage"="storage"; "PbsChangeDetectionMode"="pbs-change-detection-mode"; "Bwlimit"="bwlimit"; "Exclude"="exclude"; "PruneBackups"="prune-backups"; "Script"="script"; "Lockwait"="lockwait"; "Compress"="compress"; "Stdout"="stdout"; "Stopwait"="stopwait"; "Vmid"="vmid"; "Pigz"="pigz"; "NotificationPolicy"="notification-policy"
        }
...
function ConvertFrom-PVEJsonToPOSTNodesVzdumpRB {
    ...
    Process {
        ...
        $AllProperties = ("tmpdir", "mode", "protected", "notification-mode", "maxfiles", "all", "notification-target", "mailto", "ionice", "stop", "zstd", "mailnotification", "performance", "node", "notes-template", "job-id", "fleecing", "dumpdir", "exclude-path", "remove", "stdexcludes", "pool", "quiet", "storage", "pbs-change-detection-mode", "bwlimit", "exclude", "prune-backups", "script", "lockwait", "compress", "stdout", "stopwait", "vmid", "pigz", "notification-policy")
