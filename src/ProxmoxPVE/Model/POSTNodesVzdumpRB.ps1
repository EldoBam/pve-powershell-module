...
function Initialize-PVEPOSTNodesVzdumpRB {
    ...
    Process {
        ...
        if ($Ionice -and $Ionice -gt 8) {
		 ...
		 $DisplayNameMapping =@{
			"Pool"="pool"; "NotificationMode"="notification-mode"; "JobId"="job-id"; "Stdout"="stdout"; "NotificationPolicy"="notification-policy"; "Zstd"="zstd"; "Mode"="mode"; "Pigz"="pigz"; "Quiet"="quiet"; "Storage"="storage"; "Mailto"="mailto"; "Bwlimit"="bwlimit"; "Tmpdir"="tmpdir"; "Stopwait"="stopwait"; "Compress"="compress"; "PbsChangeDetectionMode"="pbs-change-detection-mode"; "Lockwait"="lockwait"; "Maxfiles"="maxfiles"; "Fleecing"="fleecing"; "Stop"="stop"; "ExcludePath"="exclude-path"; "Performance"="performance"; "Protected"="protected"; "Dumpdir"="dumpdir"; "NotesTemplate"="notes-template"; "Stdexcludes"="stdexcludes"; "PruneBackups"="prune-backups"; "NotificationTarget"="notification-target"; "Vmid"="vmid"; "All"="all"; "Exclude"="exclude"; "Remove"="remove"; "Mailnotification"="mailnotification"; "Script"="script"; "Ionice"="ionice"
        }
...
function ConvertFrom-PVEJsonToPOSTNodesVzdumpRB {
    ...
    Process {
        ...
        $AllProperties = ("pool", "notification-mode", "job-id", "stdout", "notification-policy", "zstd", "mode", "pigz", "quiet", "storage", "mailto", "bwlimit", "tmpdir", "stopwait", "compress", "pbs-change-detection-mode", "lockwait", "maxfiles", "fleecing", "stop", "exclude-path", "performance", "protected", "dumpdir", "notes-template", "stdexcludes", "prune-backups", "notification-target", "vmid", "all", "exclude", "remove", "mailnotification", "script", "ionice")
