...
function Initialize-PVEAccessUsersGETInner {
    ...
    Process {
        ...
        if (!$Firstname -and $Firstname.length -gt 1024) {
		 ...
		 $DisplayNameMapping =@{
			"Enable"="enable"; "RealmType"="realm-type"; "Comment"="comment"; "Email"="email"; "Groups"="groups"; "TfaLockedUntil"="tfa-locked-until"; "TotpLocked"="totp-locked"; "Expire"="expire"; "Tokens"="tokens"; "Keys"="keys"; "Userid"="userid"; "Lastname"="lastname"; "Firstname"="firstname"
        }
...
function ConvertFrom-PVEJsonToAccessUsersGETInner {
    ...
    Process {
        ...
        $AllProperties = ("enable", "realm-type", "comment", "email", "groups", "tfa-locked-until", "totp-locked", "expire", "tokens", "keys", "userid", "lastname", "firstname")
