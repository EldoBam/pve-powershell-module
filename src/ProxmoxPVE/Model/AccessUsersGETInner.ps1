...
function Initialize-PVEAccessUsersGETInner {
    ...
    Process {
        ...
        if (!$Userid -and $Userid.length -gt 64) {
		 ...
		 $DisplayNameMapping =@{
			"TotpLocked"="totp-locked"; "Email"="email"; "Lastname"="lastname"; "Tokens"="tokens"; "Enable"="enable"; "Firstname"="firstname"; "Comment"="comment"; "RealmType"="realm-type"; "TfaLockedUntil"="tfa-locked-until"; "Groups"="groups"; "Keys"="keys"; "Userid"="userid"; "Expire"="expire"
        }
...
function ConvertFrom-PVEJsonToAccessUsersGETInner {
    ...
    Process {
        ...
        $AllProperties = ("totp-locked", "email", "lastname", "tokens", "enable", "firstname", "comment", "realm-type", "tfa-locked-until", "groups", "keys", "userid", "expire")
