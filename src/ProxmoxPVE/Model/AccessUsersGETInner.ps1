...
function Initialize-PVEAccessUsersGETInner {
    ...
    Process {
        ...
        if (!$Userid -and $Userid.length -gt 64) {
		 ...
		 $DisplayNameMapping =@{
			"TfaLockedUntil"="tfa-locked-until"; "Tokens"="tokens"; "Lastname"="lastname"; "Firstname"="firstname"; "Expire"="expire"; "Email"="email"; "Keys"="keys"; "Comment"="comment"; "Enable"="enable"; "RealmType"="realm-type"; "TotpLocked"="totp-locked"; "Userid"="userid"; "Groups"="groups"
        }
...
function ConvertFrom-PVEJsonToAccessUsersGETInner {
    ...
    Process {
        ...
        $AllProperties = ("tfa-locked-until", "tokens", "lastname", "firstname", "expire", "email", "keys", "comment", "enable", "realm-type", "totp-locked", "userid", "groups")
