#
# Proxmox PowerShell VE
# Generated module to access all Proxmox VE Api Endpoints. This module has been generated from the proxmox api description v. 9.0.8
# Version: 9.0.8
# Contact: mail@timo-wolf.de
# Generated by OpenAPI Generator: https://openapi-generator.tech
#

Describe -tag 'ProxmoxPVE' -name 'PVEPOSTClusterNotificationsEndpointsWebhookRB' {
    Context 'PVEPOSTClusterNotificationsEndpointsWebhookRB' {
        It 'Initialize-PVEPOSTClusterNotificationsEndpointsWebhookRB' {
            # a simple test to create an object
            #$NewObject = Initialize-PVEPOSTClusterNotificationsEndpointsWebhookRB -Url "TEST_VALUE" -Disable "TEST_VALUE" -Header "TEST_VALUE" -Comment "TEST_VALUE" -Body "TEST_VALUE" -Secret "TEST_VALUE" -Name "TEST_VALUE" -Method "TEST_VALUE"
            #$NewObject | Should -BeOfType POSTClusterNotificationsEndpointsWebhookRB
            #$NewObject.property | Should -Be 0
        }
    }
}
