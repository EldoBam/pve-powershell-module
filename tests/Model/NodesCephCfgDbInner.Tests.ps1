#
# Proxmox PowerShell VE
# Generated module to access all Proxmox VE Api Endpoints. This module has been generated from the proxmox api description v. 9.1.1
# Version: 9.1.1
# Contact: mail@timo-wolf.de
# Generated by OpenAPI Generator: https://openapi-generator.tech
#

Describe -tag 'ProxmoxPVE' -name 'PVENodesCephCfgDbInner' {
    Context 'PVENodesCephCfgDbInner' {
        It 'Initialize-PVENodesCephCfgDbInner' {
            # a simple test to create an object
            #$NewObject = Initialize-PVENodesCephCfgDbInner -Level "TEST_VALUE" -Value "TEST_VALUE" -Section "TEST_VALUE" -Name "TEST_VALUE" -CanUpdateAtRuntime "TEST_VALUE" -Mask "TEST_VALUE"
            #$NewObject | Should -BeOfType NodesCephCfgDbInner
            #$NewObject.property | Should -Be 0
        }
    }
}
