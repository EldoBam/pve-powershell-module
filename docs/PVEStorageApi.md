...
### Example
```powershell
$POSTStorageRB = Initialize-POSTStorageRB -ContentDirs "MyContentDirs" -Server2 "MyServer2" -Namespace "MyNamespace" -Username "MyUsername" -Path "MyPath" -Password "MyPassword" -Blocksize "MyBlocksize" -Disable $false -IsMountpoint "MyIsMountpoint" -Transport "tcp" -Sparse $false -MasterPubkey "MyMasterPubkey" -Saferemove $false -Options "MyOptions" -Export "MyExport" -Iscsiprovider "MyIscsiprovider" -Maxfiles 0 -MaxProtectedBackups 0 -Vgname "MyVgname" -Nodes "MyNodes" -Monhost "MyMonhost" -ComstarHg "MyComstarHg" -DataPool "MyDataPool" -Volume "MyVolume" -SkipCertVerification $false -Type "btrfs" -Fingerprint "MyFingerprint" -TaggedOnly $false -Nowritecache $false -Storage "MyStorage" -Server "MyServer" -ComstarTg "MyComstarTg" -Share "MyShare" -Mountpoint "MyMountpoint" -Portal "MyPortal" -CreateSubdirs $false -Krbd $false -FsName "MyFsName" -Datastore "MyDatastore" -Bwlimit "MyBwlimit" -Format "raw" -Preallocation "off" -Base "MyBase" -SaferemoveThroughput "MySaferemoveThroughput" -Pool "MyPool" -Shared $false -PruneBackups "MyPruneBackups" -CreateBasePath $false -Thinpool "MyThinpool" -EncryptionKey "MyEncryptionKey" -Mkdir $false -Content "MyContent" -Target "MyTarget" -Domain "MyDomain" -Port 0 -Fuse $false -Subdir "MySubdir" -Authsupported "MyAuthsupported" -LioTpg "MyLioTpg" -Smbversion "default" -Nocow $false -Keyring "MyKeyring" # POSTStorageRB | Create a new storage. (optional)
...
```powershell
$Storage = "MyStorage" # String | The storage identifier.
$PUTStorageRB = Initialize-PUTStorageRB -ContentDirs "MyContentDirs" -Server2 "MyServer2" -Namespace "MyNamespace" -EncryptionKey "MyEncryptionKey" -Username "MyUsername" -Port 0 -Sparse $false -Password "MyPassword" -Blocksize "MyBlocksize" -Disable $false -IsMountpoint "MyIsMountpoint" -Transport "tcp" -MasterPubkey "MyMasterPubkey" -Options "MyOptions" -Maxfiles 0 -Nodes "MyNodes" -Monhost "MyMonhost" -ComstarHg "MyComstarHg" -Format "raw" -DataPool "MyDataPool" -SkipCertVerification $false -Fingerprint "MyFingerprint" -TaggedOnly $false -Nowritecache $false -Server "MyServer" -ComstarTg "MyComstarTg" -Mountpoint "MyMountpoint" -CreateSubdirs $false -Krbd $false -FsName "MyFsName" -Bwlimit "MyBwlimit" -MaxProtectedBackups 0 -Preallocation "off" -Saferemove $false -SaferemoveThroughput "MySaferemoveThroughput" -Pool "MyPool" -Shared $false -PruneBackups "MyPruneBackups" -CreateBasePath $false -Delete "MyDelete" -Mkdir $false -Content "MyContent" -Digest "MyDigest" -Fuse $false -Subdir "MySubdir" -Domain "MyDomain" -LioTpg "MyLioTpg" -Smbversion "default" -Nocow $false -Keyring "MyKeyring" # PUTStorageRB | Update storage configuration. (optional)
